...
export const handlePrismaError = (err: Prisma.PrismaClientKnownRequestError, res: Response) => {
    ...
    if (err.code === DbErrorCodes.RECORD_NOT_FOUND) {
        return res.status(StatusCodes.NOT_FOUND).json(
            errorResponse("NOT_FOUND", "The requested record was not found.")
        );
    }
    ...
    if (err.code === DbErrorCodes.FOREIGN_KEY_VIOLATION) {
        return res.status(StatusCodes.BAD_REQUEST).json(
            errorResponse("FOREIGN_KEY_ERROR", "Invalid reference to a related record.")
        );
    }
    ...
    return res.status(StatusCodes.INTERNAL_SERVER_ERROR).json(
        errorResponse("DB_ERROR", "Database operation failed.")
    );
};